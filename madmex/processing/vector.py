...
'''
import gdal
def rasterize_vector(target_ds,source_layer, band_list, burn_values,options=None):
    '''
    ...
    if burn_values:
        ...
        if gdal.RasterizeLayer(target_ds, band_list, source_layer, options = options)  != 0:
            raise Exception("error rasterizing layer: %s" % source_layer)
        else:
            return 0
